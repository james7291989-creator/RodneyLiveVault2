...
// Shared escrow document forge â€” reused by Digital Escrow "EXPORT PDF" and Asset Vault "Pull Contract"
const forgeAssignmentPDF = async (doc, toast) => {
  ...
  try {
    ...
    pdf.text(`AUTO-GENERATED BY SV-1500 CORE Â· NOTARY READY Â· ${doc.number}`, 300, 750, { align: "center" });
...
// Standalone PDF utility â€” hoisted so Asset Vault "PULL CONTRACT" and Digital Escrow "EXPORT PDF" share one engine.
      ...
      <div className="flex items-center justify-between">
        ...
        {geocode && (
          <div className="font-mono text-[8px] tracking-[0.15em] text-zinc-500">
            {geocode.zip ? 'ZIP ' + geocode.zip : 'NO ZIP'} Â· {geocode.lat && geocode.lon ? 'GEO LOCKED' : 'GEO PENDING'} Â· {geocode.postgis_radius_ready ? 'R 0.5MI READY' : 'R 0.5MI PREP'}
          </div>
        )}
      </div>
      <div className="mt-2 grid grid-cols-3 gap-2">
        <div className="rounded-md border border-zinc-800 bg-zinc-900 p-3">
          <div className="font-mono text-[9px] tracking-[0.2em] text-zinc-500">A Â· CASH MAO</div>
        ...
        <div className="rounded-md border border-zinc-800 bg-zinc-900 p-3">
          <div className="font-mono text-[9px] tracking-[0.2em] text-zinc-500">B Â· DSCR RENTAL</div>
        ...
        <div className="rounded-md border border-zinc-800 bg-zinc-900 p-3">
          <div className="font-mono text-[9px] tracking-[0.2em] text-zinc-500">C Â· CREATIVE</div>
...
const AssetVaultView = ({ assets, setAssets, ghostFetch, toast, setView, setSelectedAsset }) => {
  ...
  // APEX PHASE 3: AUTONOMOUS DEAL INJECTION â€” Global Engage Sniper
  const fireSniperInjection = async () => {
    ...
    try {
      ...
      // 2. Error handling â€” dry pool means the Sniper stays offline
      ...
      // 3. Mutate Database â€” lock the leads so no other user can pull the same Raw Leads
      const leadIds = leads.map(l => l.id);
      const { error: updateErr } = await supabase.from('missouri_properties').update({ status: 'Underwriting' }).in('id', leadIds);
      if (updateErr) throw updateErr;

      // 4. Mutate UI State â€” flip status locally and inject to the top of the vault
...
<ActionItem icon={X} label="PURGE ASSET" hint="Permanently delete from database" onClick={()=>{setOpen(false);engageSniper(asset.id, asset.address)}} last/>
      ...
      <div className="pointer-events-none col-span-8 -mt-1 flex justify-end gap-2 opacity-0 transition-opacity group-hover:pointer-events-auto group-hover:opacity-100">
        ...
        <QuickBtn icon={Download} label="ðŸ“„ Pull Contract" onClick={() => onPullContract(asset)}/>
              ...
              {!selected ? (
                ...
                <>
                  <div className="mb-3 border-b border-cyan-500/20 pb-2 font-mono text-[11px] tracking-[0.2em] text-cyan-500/80">
                    â•”â•â•â• ASSET {selected.id} :: {selected.address.toUpperCase()} â•â•â•â•—
                  </div>
                  {lines.map((l,i)=>(
...
const DigitalEscrowView = ({ assets, ghostFetch, toast }) => {
  ...
  React.useEffect(() => {
    ...
    // 1. Contract State Injection â€” mirrored by Asset Vault "Pull Contract" via buildAssignmentDoc
          ...
          <div className="max-h-[640px] overflow-y-auto">
            {pool.map(a => (
              <button key={a.id} onClick={()=>setSelected(a)} className={`group flex w-full items-center gap-3 border-b border-zinc-800/80 px-4 py-3 text-left transition-colors ${selected?.id===a.id?'bg-cyan-400/[0.06]':'hover:bg-cyan-400/[0.03]'}`}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm font-medium text-zinc-100">{a.address}</div>
                  <div className="font-mono text-[10px] tracking-wider text-zinc-600">{a.id} Â· MAO {fmt(Number(a.mao)||0)}</div>
                </div>
...
// APEX SUBSTRATE FLUSH: CACHE BROKEN 639247982860619532

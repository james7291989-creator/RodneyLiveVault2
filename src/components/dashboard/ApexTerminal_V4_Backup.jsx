 ...
 * Theme     :: Deep Zinc Dark Mode (zinc-950 / zinc-900 / zinc-800 Â· #00E5FF cyan)
 * ============================================================================= */
      ...
      <div className="w-full max-w-2xl overflow-hidden rounded-md border border-cyan-500/30 bg-zinc-950 shadow-[0_0_60px_-10px_rgba(0,229,255,0.45)]">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center px-6 py-20">
            ...
            <div className="mt-1 font-mono text-[10px] tracking-[0.2em] text-zinc-600">QUERYING SOS Â· NETRONLINE Â· LEXISNEXIS</div>
          </div>
        ) : (
  ...
  return (
    <div className="group relative grid grid-cols-[2.2fr_1fr_1fr_1fr_1.3fr_0.9fr_1.2fr_0.4fr] items-center gap-4 border-b border-zinc-800/80 px-6 py-3.5 transition-all hover:bg-cyan-400/[0.025]">
      ...
      <div className="min-w-0">
        ...
        <div className="font-mono text-[10px] tracking-wider text-zinc-600">{asset.entityType==='LLC'?'CORP Â· LLC':'NATURAL PERSON'}</div>
      </div>
  ...
  <div className="group rounded-md border border-zinc-800 bg-zinc-800 p-3 transition-all hover:border-cyan-500/40 hover:bg-zinc-900 hover:shadow-[0_0_18px_-6px_rgba(0,229,255,0.7)]">
    <div className="flex items-start justify-between gap-2">
      <div className="min-w-0">
        ...
        <div className="font-mono text-[9px] tracking-wider text-zinc-600">{asset.id} Â· {asset.city?.split(',')[0]}</div>
      </div>
            ...
            {queue.length===0 ? (
              ...
              <button key={a.id} onClick={()=>engage(a)} className={`group flex w-full items-center gap-3 border-b border-zinc-800/80 px-4 py-3 text-left transition-colors ${selected?.id===a.id?'bg-cyan-400/[0.06]':'hover:bg-cyan-400/[0.03]'}`}>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="font-mono text-[10px] tracking-wider text-zinc-600">{a.id} Â· {a.status}</div>
                </div>
...
const DigitalEscrowView = ({ assets, ghostFetch, toast }) => {
  ...
  React.useEffect(() => {
    ...
    // 1. Math Validation & Variable Mapping (Null Coalescing)
    const arv = Number(selected.arv) || 0;
    const rehab = Number(selected.rehab_estimate) || Number(selected.rehab) || 0;
    const mao = Math.round((arv * 0.7) - rehab);
    const safePurchasePrice = mao > 0 ? mao : 0;
    const standardAssignmentFee = 10000;
    const safeId = selected.id ? String(selected.id).substring(0, 6).toUpperCase() : 'XXXXXX';

    // 2. Contract State Injection
    const timer = setTimeout(() => {
      setDoc({
        number: 'ASGN-' + safeId + '-26',
        date: new Date().toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' }),
        buyer: 'Rodney & Sons Acquisitions LLC',
        seller: selected.owner || 'Owner of Record',
        assignee: 'TBD - Buyers List',
        property: selected.address || 'Address Not Found',
        purchase: safePurchasePrice,
        assignmentFee: standardAssignmentFee
      });
      setLoading(false);
    }, 600);
          ...
          <div className="max-h-[640px] overflow-y-auto">
            {pool.map(a => (
              <button key={a.id} onClick={()=>setSelected(a)} className={`group flex w-full items-center gap-3 border-b border-zinc-800/80 px-4 py-3 text-left transition-colors ${selected?.id===a.id?'bg-cyan-400/[0.06]':'hover:bg-cyan-400/[0.03]'}`}>
                ...
                <div className="min-w-0 flex-1">
                  <div className="truncate text-sm font-medium text-zinc-100">{a.address}</div>
                  <div className="font-mono text-[10px] tracking-wider text-zinc-600">{a.id} Â· MAO {fmt(Math.max(0, Math.round((Number(a.arv)||0)*0.7 - (Number(a.rehab_estimate)||Number(a.rehab)||0))))}</div>
                </div>
        ...
        <div className="overflow-hidden rounded-md border border-zinc-800 bg-zinc-900">
          <div className="flex items-center justify-between border-b border-zinc-800 bg-zinc-900/70 px-4 py-2.5">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={()=>toast('NOTARY DISPATCHED', doc.number)} className="inline-flex h-7 items-center gap-1.5 rounded-md border border-zinc-800 bg-zinc-900/70 px-3 font-mono text-[10px] tracking-[0.15em] text-zinc-300 hover:border-cyan-500/40 hover:text-cyan-300">
            ...
            {!selected ? (<div className="flex h-[600px] items-center justify-center font-mono text-[11px] tracking-[0.25em] text-zinc-600">/// AWAITING ASSET SELECTION TO INITIATE FORGE</div>) : loading || !doc ? (
              <div className="flex h-[600px] items-center justify-center">
                <div className="text-center">
                  ...
                  <div className="mt-1 font-mono text-[10px] tracking-[0.2em] text-zinc-600">INJECTING ASSET Â· ENTITY Â· MAO</div>
                </div>
              </div>
            ) : (
...
const WarRoomView = ({ assets }) => {
  ...
  return (
    <div className="px-8 pt-8 pb-16">
      ...
      <p className="mt-1 max-w-xl text-sm text-zinc-500">Global pipeline telemetry Â· all systems operational.</p>
        ...
        <div className="rounded-md border border-zinc-800 bg-zinc-900 p-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="mt-1 text-sm font-semibold text-zinc-200">Lead Ingestion Â· Contracts Â· Closes</div>
            </div>

...
const KanbanCard = ({ asset, canAdvance, onAdvance, engageSniper }) => (
  <div className="group rounded-md border border-gray-800 bg-[#0E0E0E] p-3 transition-all hover:border-cyan-500/40 hover:bg-[#0F1417] hover:shadow-[0_0_18px_-6px_rgba(0,229,255,0.7)]">
    ...
    <div className="mt-2 grid grid-cols-2 gap-2 rounded-md border border-gray-800/70 bg-[#0A0A0A] p-2">
      ...
      <div><div className="font-mono text-[9px] tracking-wider text-gray-500">MAO</div><div className="font-mono text-xs font-semibold text-white">{fmt(Math.max(0, Math.round((Number(asset.arv)||0)*0.7 - (Number(asset.rehab_estimate)||Number(asset.rehab)||0))))}</div></div>
    </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-[340px_1fr]">
        ...
        <div className="overflow-hidden rounded-md border border-gray-800 bg-[#0A0A0A]">
          ...
          <div className="bg-[#080808] p-6 min-h-[640px]">
            {!selected ? (<div className="flex h-[600px] items-center justify-center font-mono text-[11px] tracking-[0.25em] text-gray-600">/// AWAITING ASSET SELECTION TO INITIATE FORGE</div>) : loading || !doc ? (
...
const TopBar = ({ view }) => {
  ...
  return (
    <div className="sticky top-0 z-[100] border-b border-gray-800 bg-[#000000]">
